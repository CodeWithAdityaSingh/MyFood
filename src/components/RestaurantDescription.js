...
const RestaurantDescription = () => {
  ...
  const { itemCards } =
    menuData[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[2]?.card?.card;

  let items = menuData[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards
    .filter(
      (items) =>
    ...
    <div className="h-screen w-screen flex flex-col items-center mt-44 relative  ">
      <div className="w-6/12 text-left ">
        ...
        <div className=" border-2  rounded-3xl my-3 shadow-2xl shadow-black p-4 font-semibold ">
          ...
          <div className="ml-5 text-orange-600 font-semibold decoration-orange-600 decoration-dashed">
            {cuisines.join(",")}
          </div>
          <div className="font-semibold ml-5 my-5">
            {minDeliveryTime}-{maxDeliveryTime} Min
          </div>
          <div className="w-full border"></div>
          <div className="flex items-center font-bold text-xl">
            <img
              className="w-12 m-5 rounded-full"
              src={
