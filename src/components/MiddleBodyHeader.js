...
const MiddleBodyHeader = () => {
  ...
  return (
    <div className='flex justify-evenly h-40 w-full p-4 items-center'>
     ...
     <input className = 'h-full p-2' ref={searchItems} type='text' placeholder='Search Products' ></input>
     <button onClick={()=>{
        

        let data =   selector.filter((items)=>{
              return items.info.name.toLowerCase().includes(searchItems.current.value.toLowerCase())
        })
     ...
     }}><img className='w-12 shadow-xl border-2 p-2' src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSJ-pjNh9Ap412zXaR0gCK7NkFPB-Snmr6lsA&s'></img></button>
     </div>
    </div>
  )
}
