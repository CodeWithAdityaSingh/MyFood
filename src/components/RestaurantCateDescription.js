...
const RestaurantCateDescription = ({ data2 }) => {
  ...
  const { description, imageId, name, defaultPrice, price } = data2.card.info;
  ...
  return (
    <div className="flex  justify-between gap-8  my-6 p-5 border-b items-center w-full">
      ...
      <div className="w-3/12">
        <button
          className="absolute mt-32 ml-11 w-20 text-green-500 font-bold hover:bg-slate-200 bg-orange-100 p-2 rounded-md"
          onClick={() => {
        ...
        <img
          className="w-40 h-36 m-auto rounded-xl"
          src={
